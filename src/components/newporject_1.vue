      ...
      >
        <el-col :span="5">
          <div class="grid-content switch">
            ...
            ></el-switch> -->
            <el-button-group v-show="button1[0]">
              <el-button type="success" @click="personal()">个人用户</el-button>
              <el-button type="warning" @click="platform()">平台用户</el-button>
            </el-button-group>
            <span v-show="!button1[0]">{{swichtext}}</span>
          </div>
        </el-col>
...
export default {
  ...
  mounted() {
    ...
    this.swichtext = this.$store.state.newproject.mainSwitchText;
  ...
  methods: {
    ...
    mountOrgchart() 
    {
      ...
      {
       this.digui(this.$children[0]);
      }
    },
    digui(val) {
      if (val.$children.length !== 0 && this.orgchart === undefined)
       {
        val.$children.forEach(dataC =>
         {
          dataC.orgchart !== undefined　?  (this.orgchart = dataC.orgchart)　: this.digui(dataC);
        });
      }
    },
    //添加
    ...
    nextFunctionS() {
      ...
      this.$store.commit("NasiadActive");
      this.$store.commit("NMSBool");
    },
    //切换
    personal() {
      this.value1 = false;
      this.$store.commit("NMSwitchText", "个人用户");
      this.$store.commit("NSwitchF");
      this.$store.commit("NSasiadActive", 2);
    },
    platform() {
      this.value1 = true;
      this.$store.commit("NMSwitchText", "平台用户");
      this.$store.commit("NSwitchT");
